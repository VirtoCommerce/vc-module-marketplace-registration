...
using VcmpRegistrationModule = VirtoCommerce.MarketplaceRegistrationModule.Core;
using VendorModule = VirtoCommerce.MarketplaceVendorModule.Core;

namespace VirtoCommerce.MarketplaceRegistrationModule.Web.Authorization;
...
public static class ApplicationBuilderExtensions
{
    public static IApplicationBuilder UseModuleAuthorization(this IApplicationBuilder appBuilder)
    {
        ...
        permissionsProvider.RegisterPermissions(VcmpRegistrationModule.ModuleConstants.Security.Permissions.AllPermissions.Select(x => new Permission { GroupName = "Marketplace", Name = x }).ToArray());
    ...
    private static async Task SavePredefinedRolesAsync(RoleManager<Role> roleManager)
    {
        foreach (var vendorModuleRole in VendorModule.ModuleConstants.Security.Roles.AllRoles)
        {
            ...
            var registrationModuleRole = VcmpRegistrationModule.ModuleConstants.Security.Roles.AllRoles.Where(x => x.Id == vendorModuleRole.Id).FirstOrDefault();
            ...
            if (existingVendorModuleRole != null)
            {
                ...
                if (registrationModuleRole != null)
                {
                    vendorModuleRole.Permissions = vendorModuleRole.Permissions.Concat(registrationModuleRole.Permissions).Distinct().ToList();
                }
